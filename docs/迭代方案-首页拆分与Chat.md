# 迭代方案：首页拆分 + UI 改造 + 具身交互智能体学习 Chat
...
3. 缺少非面试场景的"学习问答"入口——用户无法就单个知识点向具身交互智能体提问

目标：拆分信息架构、建立统一设计系统、新增"和具身交互智能体聊天学习"的 Chat 界面。

## 2. 已确认决策

| # | 决策点 | 结论 |
|---|--------|------|
| 1 | chat 形态 | 文字优先 + 具身交互智能体可选侧栏（TTS 播报 / ASR 输入，可开关） |
...
| 路由 | 页面 | 说明 |
|------|------|------|
| `/` | 首页（落地页） | 产品介绍 + 功能入口（开始面试 / 与具身交互智能体聊天 / 历史记录 / 管理配置） |
| `/setup` | 面试配置 | 岗位/难度/时长/风格选择 + **简历管理（上传/列表/删除/轻量分析）** |
| `/interview` | 面试进行 | 具身交互智能体 + 对话 + 回答输入（沉浸式，无顶部导航） |
| `/chat` | 具身交互智能体学习 Chat | 三栏：左会话列表 / 中对话流 / 右可选具身交互智能体侧栏 |
...
4. **具身交互智能体接入 chat**：复用现有 `AvatarProvider.speak()`；TTS 默认关、用户手动开；文字渲染不被 TTS 阻塞。
5. **范围较大**：分 5 步实施（见下）。
...
5. **chat 具身交互智能体侧栏**：TTS/ASR 接入（第 3 点增强）
