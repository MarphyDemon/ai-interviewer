...
<template>
  <div class="mx-auto max-w-2xl px-4 py-10">
    ...
    <h1 class="mb-8 text-2xl font-bold text-gray-900">{{ t('setup.title') }}</h1>
    ...
    <div class="card mb-6">
      ...
      <div v-if="resumeStore.resumes.length === 0" class="mt-3 text-sm text-gray-400">{{ t('home.noResumes') }}</div>
    ...
    <div class="card mb-6">
      ...
      <button
        ...
        class="w-full rounded-xl border-2 border-dashed border-primary-200 px-4 py-4 text-sm text-primary-600 transition hover:border-primary-400 hover:bg-primary-50/50 disabled:opacity-60"
      >
        {{ jdUploading ? '上传中...' : '上传 JD 文件（PDF / Markdown / Word）' }}
      </button>

      <!-- JD 列表 -->
      <div v-if="jdStore.jds.length === 0" class="mt-3 text-sm text-gray-400">暂无 JD，可粘贴或上传</div>
      <div v-else class="mt-3 space-y-2">
        <div
        ...
        >
          ...
          <button @click="jdStore.deleteJd(jd.id)" class="ml-2 text-sm text-red-400 transition hover:text-red-600">
            删除
          </button>
        </div>
      </div>
    ...
    <div class="card space-y-6">
      ...
      <button @click="start" :disabled="starting" class="btn-primary w-full">
